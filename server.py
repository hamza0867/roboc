...
class MazeDisplayer:
    def __init__(self, labyrinthe, clients):
        labyrinthe.register_observer(self)
        self.clients = clients

    def update(self, labyrinthe):
        for conn in self.clients:
            message = my_message.Message(
                str(labyrinthe), my_message.MessageType.MAZE)
...
def start_Server():
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        maze_displayer = MazeDisplayer(maze, clients)

        maze_displayer.update(maze)
